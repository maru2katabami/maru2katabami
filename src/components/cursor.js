...
export const Cursor = () => {
  ...
  const { TEXTURE_DOWNSAMPLE, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE_DISSIPATION, PRESSURE_ITERATIONS, CURL, SPLAT_RADIUS, OPACITY } = zusCursor()
  useEffect(() => {
    ...
    window.addEventListener("mousedown", () => { pointers[0].down = true; pointers[0].color = [Math.random() * OPACITY, Math.random() * OPACITY, Math.random() * OPACITY] })
    ...
    window.addEventListener("touchstart", e => {
      ...
      for (let i = 0; i < e.targetTouches.length; i++) {
        ...
        pointers[i].color = [Math.random() * OPACITY, Math.random() * OPACITY, Math.random() * OPACITY]
      }
    })
